...
Matching and searching can be done with globbing or regular expression pattern
matching or simply by searching for strings or substrings. These can also be
combined. You can enter multiple patterns at one `Find:` or `Pick:`
prompt. However, be aware, that if two patterns are entered at the same prompt
that match the same thing they will cancel each other out, due to the toggling
nature of the resulting action.
